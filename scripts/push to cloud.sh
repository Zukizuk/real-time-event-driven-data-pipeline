...
# Validate
aws ecs run-task --cluster ecommerce-pipeline-cluster --task-definition validate-task --launch-type FARGATE --network-configuration "awsvpcConfiguration={subnets=[subnet-0d8436345ee46b7b8,subnet-0c258e856f0e04aca ],securityGroups=[sg-0222dcff8c650af7c],assignPublicIp=ENABLED}" --region eu-west-1
        ...
        "containerOverrides": [
            {
                ...
                "command": [
                    "s3://your-bucket/path/to/input1.csv",
                    "s3://your-bucket/path/to/input2.csv",
                    "s3://your-bucket/path/to/output.csv"
                ]
            }
        ]
    }'
